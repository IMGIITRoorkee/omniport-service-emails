...
def email_push(
    ...
    if by is not None:
        ...
        sender = ''
        html_content_mod = html_content.replace("<div class='post1' style='font-size: 14px;color: #293B52;'><b>Posted By:</b> Sender/Text</div>", sender)
    ...
    msg = EmailMessage(
        ...
        body=html_content_mod.replace("Subject/Text", subject_text).replace("Body/Text", body_text).replace("TargetApp/Text", target_name).replace("TargetURL/Text", target_url).replace("Time/Text", now.strftime("%d %B %Y at %H:%M")),
